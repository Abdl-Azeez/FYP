...
if (isset($_POST['Formsubmit']))
{
  ...
  $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, request.Position, request.status,
   users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance, request.ActionDate FROM request INNER JOIN users ON users.Department = request.Departments WHERE users.StaffName = request.StaffName AND request.status>$status AND users.roles='staff'
   AND  request.StaffName='$empname' AND request.leaveType='$empleave' ORDER BY  request.AdminDate desc";
  }
  else if (($empleave==5) && ($empname!=5)){
    $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, request.Position, request.status,
     users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance, request.ActionDate FROM request INNER JOIN users ON users.Department = request.Departments WHERE users.StaffName = request.StaffName AND request.status>$status AND users.roles='staff'
     AND  request.StaffName='$empname' ORDER BY  request.AdminDate desc";
  }
  else if (($empleave!=5) && ($empname==5)){
    $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, request.Position, request.status,
     users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance, request.ActionDate FROM request INNER JOIN users ON users.Department = request.Departments WHERE users.StaffName = request.StaffName AND request.status>$status AND users.roles='staff'
     AND request.leaveType='$empleave' ORDER BY  request.AdminDate desc";
    }
  else if (($empleave==5) && ($empname==5)){
    $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, request.Position, request.status,
     users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance, request.ActionDate FROM request INNER JOIN users ON users.Department = request.Departments WHERE users.StaffName = request.StaffName AND request.status>$status AND users.roles='staff'
     ORDER BY  request.AdminDate desc";
    }
  ...
  if($row==''){
    ...
    while($row = mysqli_fetch_array($result))
    {
      ...
      else if ($leavestatus==3){$status="Not Recommended";}
