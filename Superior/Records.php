    ...
    <navigation  class='fixed-top navbar navbar-expand-lg navbar-dark nav-style'>
      ...
      <button class='navbar-toggler' type='button' data-toggle='collapse' data-target='#navbarNav' aria-controls='navbarNav' aria-expanded='false' aria-label='Toggle navigation'>
        ...
        <ul class='ul-class nav navbar-nav borderEffect'>
         <li class='activated'><a href='\fyp\Superior\Records.php' style='color: #121307; padding-left:8px;' class="ml-2">Staff Requests</a></li>
    ...
    <div class='component'>
      ...
      <div style="display: flex;justify-content: space-around;width: 100%;margin-top: 50px;">
        ...
        <?php
          ...
          $user=$_SESSION['username'];
          $status=0;
          $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, users.Department, request.status FROM request
          INNER JOIN users ON users.Department = request.Departments WHERE users.StaffName=request.StaffName AND users.roles='Staff' AND request.status=$status ORDER BY  request.PostingDate desc";
          $result= mysqli_query ($connection, $sql) or die ("Could not execute query");
