<?php
  include'session.php';
?>
<title>L_M_S | STAFFS RECORDS PAGE</title>
    ...
    <navigation  class='fixed-top navbar navbar-expand-lg navbar-dark nav-style'>
      ...
      <button class='navbar-toggler' type='button' data-toggle='collapse' data-target='#navbarNav' aria-controls='navbarNav' aria-expanded='false' aria-label='Toggle navigation'>
        ...
        <ul class='ul-class nav navbar-nav borderEffect'>
         ...
         <li class='activated'><a href='\fyp\Superior\StaffRecords.php' style='color: #121307;'>Records</a></li>
      ...
      <div style="display: flex;justify-content: space-around;width: 100%;margin-top: 50px;">
        ...
        <?php
          ...
          $sql= "SELECT request.Id, request.StaffName, request.StaffID, request.leaveType, request.DateFrom, request.DateTo, request.Attachment, request.PostingDate, request.Position, request.status,
           users.ELtaken, users.ELbalance, users.CLtaken, users.CLbalance, users.Annualtaken, users.Annualbalance, users.OLtaken, users.OLbalance FROM request INNER JOIN users ON users.Department = request.Departments WHERE users.StaffName = request.StaffName AND request.status>$status AND users.roles='staff' ORDER BY  request.ActionDate desc";
          $result= mysqli_query ($connection, $sql) or die ("Could not execute query");
          ...
          if ($myrow[0]>0){
              ...
              echo"
                      <tbody>
                        <tr>
                          ...
                          <td>$myrow[4]</td>
                          <td>$myrow[5]</td>
                          <td style=\"max-width:150px;\">$myrow[7]</td>
            ...
            echo "<tbody><tr><td colspan='11'><span class='NoRecord'>No Records Available</span></td></tr></tbody></table>";
          }
      ?>
