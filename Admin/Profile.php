  ...
  <navigation  class='fixed-top navbar navbar-expand-lg navbar-dark nav-style'>
    ...
    <button class='navbar-toggler' type='button' data-toggle='collapse' data-target='#navbarNav' aria-controls='navbarNav' aria-expanded='false' aria-label='Toggle navigation'>
      ...
      <ul class='ul-class nav navbar-nav borderEffect'>
        ...
        <li class='activated'><a href='\fyp\Admin\Profile.php' style='color: #121307;'>Profile</a></li>
      ...
      echo "
        <form id=\"Profile\" enctype=\"multipart/form-data\" class='profileSection' method='post' action=\"#\" onsubmit=\"return Profile();\">
          ...
          <div class='profilePicture'>
            <a href=\"";if ($myrow[22]==""){echo "\FYP\images\userPlaceholder.png";}else{echo"\FYP\Admin\ProfilePictures\\$myrow[22]";};echo"\" target=\"_blank\">
              <img src='";if ($myrow[22]==""){echo "\FYP\images\userPlaceholder.png";}else{echo"\FYP\Admin\ProfilePictures\\$myrow[22]";};echo"'/>
            </a>
...
$connection = mysqli_connect("localhost", "root", "", "FYP") or die ("Couldn't connect to server");
$user=$_SESSION['username'];
$sql= "SELECT * FROM users WHERE roles='Admin' AND username='$user'";
$result= mysqli_query ($connection, $sql) or die ("Could not execute query");
$myrow= mysqli_fetch_row ($result);
?>
...
function rowdel(){
  if(confirm("Are you sure you want to remove your profile picture?")){
    ...
    ({
        type:'post',
        url:'/FYP/Admin/Picturedel.php?empid=<?php echo "$myrow[0]";?>',
        data:{
...
function Profile()
{
   ...
   var r = confirm("You are about to update your profile\n Press OK to proceed \nCANCEL to terminate the process");
    if (r == true) {
      ...
      ({
          type:'post',
          url:'/FYP/Admin/UpdateProfile.php',
          data:formdata,
          contentType: false,
          processData: false,
          success: function (response) {
